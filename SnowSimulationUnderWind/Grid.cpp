...
void Grid::getFlagsFromGridNodes(CellType* flag) {
...
void Grid::updateVelocities(LatticeBoltzmann* boltzmann, CellType* flags) const{
	...
	for (int i=0; i<obj->size; i++){
		...
		float max_vel = 0;
		int w = 4;
		for (int xx = ox - w; xx < ox + w; xx++) {
			for (int yy = oy - w; yy < oy + w; yy++) {

				if (xx < 0 || xx >= NX || yy < 0 || yy >= NY || (xx == ox && yy == oy))  continue;
				int id = (int)(xx + yy * NX);
				if (nodes[id].active) continue;

				if (boltzmann->U[id].length_squared() > max_vel) {
					v = boltzmann->U[id] * 0.5;
					max_vel = boltzmann->U[id].length_squared();
				}
			}
		}
		...
		p.velocity = flip*FLIP_PERCENT + pic*(1-FLIP_PERCENT) + v;
